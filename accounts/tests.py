...
class TestSignupView(TestCase):
    ...
    def test_failure_post_with_empty_username(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(User.objcts.all().count(), 0)
    ...
    def test_failure_post_with_duplicated_user(self):
        ...
        response = self.client.post(self.url, data=depublicated_data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(User.objects.all().count(), 1)

        context = response.context
        form = context["form"]
        self.assertFalse(form.is_vald())
    ...
    def test_failure_post_with_too_short_password(self):
        ...
        form = context["form"]
        self.assertFalse(form.is_valid())
        self.assertEqual(form.errors["password2"][0], "このパスワードは短すぎます。最低8文字以上必要です。")

    def test_failure_post_with_password_similar_to_username(self):
        password_similar_to_username_data = {
            "username": "testuser",
            "email": "testmail@exampla.com",
            "password1": "testuserr",
            "password2": "testuserr",
        }
